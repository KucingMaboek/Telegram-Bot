...
public class Query {
    ...
    public void updateData(String table, String column, String chatId, String text, String id) {
        String query = String.format("UPDATE `%s` SET `%s`= \'%s\' WHERE  id = (SELECT MAX(id) FROM `%s` WHERE id= \'%s\');", table, column, text, table, id);
        try (Statement stats = Helper.connectDatabase().createStatement()) {
            stats.executeUpdate(query);
        } catch (SQLException e) {
            e.printStackTrace();
